...
- item (allow) permissions are inherited recursively through the group tree
  - example: a user can see all computers under a specific group he has `"item":{"read":true}` rights for, even if the computer is inside a subgroup where he has no `read` rights for
