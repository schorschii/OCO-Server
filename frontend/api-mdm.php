...
if($path === '/profile') {
	...
	$db->updateMobileDevice(
		...
		$md->unlock_token, $md->info, $md->policy, $md->parameters, $md->notes, $md->force_update, true/*last_update*/
	);
	...
	switch($request['MessageType'] ?? null) {
		...
		case 'CheckOut':
			...
			$db->updateMobileDevice(
				...
				null/*unlock_token*/, $md->info, $md->policy, $md->parameters, $md->notes, $md->force_update, true/*last_update*/
			);
			break;
	...
	if(isset($request['Status']) && isset($request['CommandUUID'])) {
		...
		if($mdc) {
			// do not throw an exception in case the command UUID cannot be found (e.g. deleted from database)
			// iOS will retry it infinitely, and so we are unable to manage the device in this case
			...
			$db->updateMobileDevice(
				...
				$md->unlock_token, $md->info, $md->policy, $md->parameters, $md->notes, $md->force_update
			);
			...
			if($state === Models\MobileDeviceCommand::STATE_SUCCESS) {
				if($rt === 'DeviceInformation') {
					...
					$db->updateMobileDevice(
						...
						$request['QueryResponses']['DeviceName']??$md->device_name??'?',
						$request['QueryResponses']['SerialNumber']??$md->serial,
						$md->vendor_description, $product??$md->model, $os??$md->os, $md->device_family, $md->color,
						$md->profile_uuid, $md->push_token, $md->push_magic, null/*push_sent*/,
						$md->unlock_token, json_encode($request['QueryResponses'],JSON_PARTIAL_OUTPUT_ON_ERROR), $md->policy, $md->parameters, $md->notes, $md->force_update, true/*last_update*/
					);
