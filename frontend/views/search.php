...
<?php foreach($db->getAllDomainuserByName($_GET['query'], 5) as $u) { $counter ++; ?>
	<div class='node'>
		<a onkeydown='handleSearchResultNavigation(event)' onclick='event.preventDefault();closeSearchResults();refreshContentDomainuserDetail("<?php echo $u->id; ?>")' href='<?php echo explorerLink('views/domainuser-detail.php?id='.$u->id); ?>'><img src='img/users.dyn.svg'><?php echo htmlspecialchars($u->username); ?></a>
	</div>
<?php } ?>
