...
function echoTargetComputerGroupOptions($parent=null) {
	...
	foreach($db->selectAllComputerGroupByParentComputerGroupId($parent) as $cg) {
		...
		echo "<label class='blockListItem loadSubList' target='views/partial/computer-selection-items.php?single=".$SINGLE_SELECTION."&computer_group_id=".$cg->id."'>";
...
function echoTargetComputerReportOptions() {
	...
	foreach($db->selectAllReport() as $r) {
		...
		echo "<label class='blockListItem loadSubList' target='views/partial/computer-selection-items.php?single=".$SINGLE_SELECTION."&computer_group_id=".$r->id."'>";
		if($CONTAINER_SELECTION)
...
<div class='box listItems'>
	<label class='blockListItem big noSearch loadSubList' target='views/partial/computer-selection-items.php?single=<?php echo $SINGLE_SELECTION; ?>&computer_group_id=-1' tabindex='0' onkeypress='if(event.key=="Enter"){this.click()}'><?php echo LANG('all_computers'); ?><img src='img/eye.dyn.svg' class='dragicon'></label>
