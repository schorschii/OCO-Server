...
if(!isset($SINGLE_SELECTION))
	$SINGLE_SELECTION = 0;

function echoTargetDomainUserGroupOptions($parent=null) {
	global $db, $cl, $CONTAINER_SELECTION, $SINGLE_SELECTION;
	...
	foreach($db->selectAllDomainUserGroupByParentDomainUserGroupId($parent) as $pg) {
		...
		echo "<label class='blockListItem loadSubList' target='views/partial/domain-user-selection-items.php?single=".$SINGLE_SELECTION."&domain_user_group_id=".$pg->id."'>";
...
function echoTargetDomainUserReportOptions() {
	...
	foreach($db->selectAllReport() as $r) {
		...
		echo "<label class='blockListItem loadSubList' target='views/partial/domain-user-selection-items.php?single=".$SINGLE_SELECTION."&domain_user_report_id=".$r->id."'>";
		if($CONTAINER_SELECTION)
...
<div class='box listItems'>
	<label class='blockListItem big noSearch loadSubList' target='views/partial/domain-user-selection-items.php?single=<?php echo $SINGLE_SELECTION; ?>&domain_user_group_id=-1' tabindex='0' onkeypress='if(event.key=="Enter"){this.click()}'><?php echo LANG('all_domain_users'); ?><img src='img/eye.dyn.svg' class='dragicon'></label>
