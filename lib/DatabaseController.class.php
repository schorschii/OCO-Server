...
class DatabaseController {
	...
	*/
	...
	public function updateMobileDevice($id, $udid, $state, $device_name, $serial, $vendor_description, $model, $os, $device_family, $color, $profile_uuid, $push_token, $push_magic, $push_sent, $unlock_token, $info, $policy, $parameters, $notes, $force_update, $last_update=false) {
		if($last_update === true)
			$last_update = date('Y-m-d H:i:s', time());
		$this->stmt = $this->dbh->prepare(
			'UPDATE mobile_device SET udid=:udid, state=:state, device_name=:device_name, serial=:serial, vendor_description=:vendor_description, model=:model, os=:os, device_family=:device_family, color=:color, profile_uuid=:profile_uuid, push_token=:push_token, push_magic=:push_magic, push_sent=:push_sent, unlock_token=:unlock_token, info=:info, policy=:policy, parameters=:parameters, notes=:notes, force_update=:force_update'
	...
	public function selectPackage($id, $binaryAsBase64=false) {
		if($binaryAsBase64 === null) { // do not fetch icons if not necessary
			$this->stmt = $this->dbh->prepare(
				'SELECT p.*, pf.name AS "package_family_name", su.username AS "created_by_system_user_username", su2.username AS "updated_by_system_user_username",
				(SELECT COUNT(cp2.id) FROM computer_package cp2 WHERE cp2.package_id = p.id) AS "install_count"
				FROM package p
				INNER JOIN package_family pf ON pf.id = p.package_family_id
				LEFT JOIN system_user su ON su.id = p.created_by_system_user_id
			...
			$this->stmt = $this->dbh->prepare(
				'SELECT p.*, pf.name AS "package_family_name", pf.icon AS "package_family_icon", su.username AS "created_by_system_user_username", su2.username AS "updated_by_system_user_username",
				(SELECT COUNT(cp2.id) FROM computer_package cp2 WHERE cp2.package_id = p.id) AS "install_count"
				FROM package p
				INNER JOIN package_family pf ON pf.id = p.package_family_id
				LEFT JOIN system_user su ON su.id = p.created_by_system_user_id
	...
	private function insertOrUpdateDomainUser($uid, $username, $display_name) {
		...
		$this->stmt = $this->dbh->prepare(
			...
			WHERE (uid IS NOT NULL AND uid = :uid) OR (:uid IS NULL AND username = :username) LIMIT 1'
		);
	...
	public function executeReport($id) {
		...
		if(!preg_match('/^SELECT\b/i', ltrim($report->query))) {
